...
public class KeyPressed implements KeyListener {
    ...
    public void keyPressed(KeyEvent e){
        ...
        if (!game.isRunning()) {
            ...
            if(!game.getContentPane().equals(game.getMapPanel())){//check to make sure the user sees the panel they are on
                return;
            }
            //set the panel
            if(game.isPaused()){
                    ...
                    if(talk){//if they are looking at each other create dialogbox
                        DialogBox box = new DialogBox(game, npc.getName() + ": " + npc.getDialog(), game.getGlobalTextSpeed()){
                            ...
                            public void onComplete(){
                                if(npc.getName().equalsIgnoreCase("Owner")){//if talking to owner set panel to pokemart
